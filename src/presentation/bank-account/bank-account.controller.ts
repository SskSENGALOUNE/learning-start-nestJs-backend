...
import { IndexCompareQuery } from 'src/application/bank-account/queries/index-compare.query/index-compare.query';
import { IndexCompareDto } from './dto/index-compare.dto';
import { GetCompositeFilterQuery } from 'src/application/bank-account/queries/get-composite-filter/get-composite-filter.query';
import { AccountType } from 'generated/prisma/enums';
import { GetCompositeFilterDto } from './dto/get-composite-filter.dto';
...
export class BankAccountController {
    ...
    getWithOffset(@Query() dto: GetAccountsOffsetDto) {
        return this.queryBus.execute(
            new GetAccountsOffsetQuery(dto.page ?? 1, dto.limit ?? 20),
        );
    }
    ...
    @Get('stats')
    getStats() {
        return this.queryBus.execute(new GetAccountStatsQuery());
    }
    ...
    @Get('index-compare')
    indexCompare(@Query() dto: IndexCompareDto) {
        return this.queryBus.execute(
            new IndexCompareQuery(
                dto.bankName ?? 'BCEL',
                dto.rate ?? 3,
                dto.limit ?? 50,
            ),
    ...
    @Get('composite')
    getCompositeFilter(@Query() dto: GetCompositeFilterDto) {
        return this.queryBus.execute(
            new GetCompositeFilterQuery(
                dto.accountType ?? AccountType.SAVINGS,
                dto.isActive ?? true,
                dto.limit ?? 50,
            ),
        );
